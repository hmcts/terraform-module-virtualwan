...
resource "azurerm_vpn_gateway_connection" "vpn_gateway_connections" {
  ...
  dynamic "vpn_link" {
    ...
    content {
      ...
      vpn_site_link_id               = azurerm_vpn_site.vpn_site[lookup(each.value, "remote_vpn_site_name", null)].link[[for key, value in var.resource_groups: key if value.name == vpn_link.vpn_site_link_name]].id
